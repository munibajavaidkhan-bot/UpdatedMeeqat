...
<div class="flex justify-end mb-6">
    <a href="{{ route('admin.meeqat-locations.create') }}" class="h-11 px-5 rounded-xl bg-primary-500 text-white text-sm font-semibold hover:bg-primary-600 transition-all duration-200 shadow-sm whitespace-nowrap inline-flex items-center justify-center">+ Add Location</a>
</div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-primary-100">
                    ...
                    <th class="px-5 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Name</th>
                    <th class="px-5 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Arabic</th>
                    <th class="px-5 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Coordinates</th>
            ...
            <tbody>
                @foreach($locations as $loc)
                    <tr class="border-b border-primary-50 last:border-0 hover:bg-primary-50/30 transition-colors">
                        <td class="px-5 py-4 text-muted">{{ $loc->sort_order }}</td>
                        <td class="px-5 py-4">
                            <span class="text-heading font-medium text-sm">{{ $loc->name_en }}</span>
                        </td>
                        <td class="px-5 py-4 arabic text-primary-600">{{ $loc->name_ar }}</td>
                        <td class="px-5 py-4 font-mono text-muted text-xs">{{ $loc->latitude }}, {{ $loc->longitude }}</td>
                        <td class="px-5 py-4">
                            @if($loc->is_active)
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.meeqat-locations.edit', $loc) }}" class="p-2 rounded-lg bg-primary-50 hover:bg-primary-500 text-primary-600 hover:text-white border border-primary-100 hover:border-primary-500 transition-all duration-200">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="{{ route('admin.meeqat-locations.destroy', $loc) }}" method="POST" onsubmit="return confirm('Delete?')">
                                    @csrf @method('DELETE')
