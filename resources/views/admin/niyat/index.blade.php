...
<div class="flex justify-end mb-6">
    <a href="{{ route('admin.niyat.create') }}" class="h-11 px-5 rounded-xl bg-primary-500 text-white text-sm font-semibold hover:bg-primary-600 transition-all duration-200 shadow-sm whitespace-nowrap inline-flex items-center justify-center">+ Add New Niyat</a>
</div>
...
<div class="bg-white rounded-2xl border border-primary-100 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-primary-100">
                    ...
                    <th class="px-5 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Type</th>
            ...
            <tbody>
                @forelse($niyats as $niyat)
                    <tr class="border-b border-primary-50 last:border-0 hover:bg-primary-50/30 transition-colors">
                        <td class="px-5 py-4 text-muted text-xs">{{ $niyat->id }}</td>
                        <td class="px-5 py-4 text-heading font-medium">{{ Str::limit($niyat->title_en, 40) }}</td>
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg bg-primary-50 text-primary-700 text-xs font-semibold border border-primary-200 capitalize whitespace-nowrap">{{ $niyat->type }}</span>
                        </td>
                        <td class="px-5 py-4 arabic text-primary-600 text-sm">{{ Str::limit($niyat->arabic_text, 30) }}</td>
                        <td class="px-5 py-4">
                            @if($niyat->is_active)
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.niyat.edit', $niyat) }}" class="p-2 rounded-lg bg-primary-50 hover:bg-primary-500 text-primary-600 hover:text-white border border-primary-100 hover:border-primary-500 transition-all duration-200">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="{{ route('admin.niyat.destroy', $niyat) }}" method="POST" onsubmit="return confirm('Delete this niyat?')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-10 text-muted">
                            Koi niyat nahi mili.
                            <a href="{{ route('admin.niyat.create') }}" class="text-primary-500 hover:underline ml-1 font-medium">Add first niyat →</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="p-5 border-t border-primary-100">{{ $niyats->links() }}</div>
</div>
